...
class PostController extends Controller
{
    ...
    public function update(Request $request, int $post): JsonResponse
    {
        try {
            ...
            // Chỉ owner hoặc admin được phép chỉnh sửa.
            $isOwner = $foundPost->user_id === $authUser->id;
            $isAdmin = $authUser->role === 'admin';
            ...
            // Update caption nếu có
            if ($request->has('caption') && $request->input('caption') !== null) {
                $foundPost->caption = $request->input('caption');
            }

            // Handle image update
            if ($request->hasFile('image')) {
                $imageFile = $request->file('image');
                $newImagePath = $this->storePublicFileWithOriginalName(
                    $imageFile,
                    $this->postImageDirectory($foundPost->id)
                );

                if ($hasPostImagesTable) {
                    // Delete old images and create new one
                    $oldImages = $foundPost->images()->pluck('image_url')->all();
                    $foundPost->images()->delete();
                    
                    // Create new image record
                    $foundPost->images()->create([
                    ...
                    // Legacy schema - update posts.image field
                    if ($hasLegacyImageColumn && ! empty($foundPost->image)) {
                        Storage::disk('public')->delete($foundPost->image);
                    }
                    $foundPost->image = $newImagePath;
                }
            }

            $foundPost->save();
            $foundPost = $foundPost->load('user:id,username,email,avatar');

            if ($hasPostImagesTable) {
                $foundPost->load('images');
            } else {
                $this->attachLegacyImages($foundPost);
            }
